...
from .new_encoder_decoder import decoder
#from .new_encoder_decoder import encoder
from .ftt import FTT
from .mymodality3 import ModalityFusion
from .vgg_perceptual_loss import VGGPerceptualLoss
from .transformers import *
from torch.autograd import Variable
from models_SDT.encoder_part import Encoder_part
...
class ModelMain(nn.Module):
    ...
    def __init__(self, opts, mode='train'):
        super().__init__()
        self.opts = opts
        # self.img_encoder = ImageEncoder(img_size=opts.img_size, input_nc=opts.ref_nshot, ngf=opts.ngf,
        #                                 norm_layer=nn.LayerNorm)
        #self.img_decoder = ImageDecoder(img_size=opts.img_size, input_nc=opts.bottleneck_bits + opts.char_num,
        #                                output_nc=1, ngf=opts.ngf, norm_layer=nn.LayerNorm)
        self.img_encoder = Encoder_part()
        #self.img_encoder = encoder(4, 64, 16)
        self.img_decoder = decoder(2, 16)
        self.vggptlossfunc = VGGPerceptualLoss()
        self.modality_fusion = ModalityFusion(img_size=opts.img_size, ref_nshot=opts.ref_nshot,
                                              bottleneck_bits=opts.bottleneck_bits, ngf=opts.ngf, mode=opts.mode,skip=False)
    ...
    def forward(self, data, mode='train'):
        ...
        # img encoding
        img_feat, NCELOSS_IMG = self.img_encoder(ref_img)
         
    
        # seq encoding
        ref_img_ = ref_img.view(ref_img.size(0) * ref_img.size(1), ref_img.size(2), ref_img.size(3)).unsqueeze(
            -1)  # [max_seq_len, n_bs * n_ref, 9]
        seq_feat, _ = self.transformer_main(ref_img_, ref_seq_cat,
                                            mask=ref_pad_mask)  # [n_bs * n_ref, max_seq_len + 1, 9]
        ...
        latent_feat_seq = self.transformer_main.att_residual(
            latent_feat_seq)  # [n_bs, max_seq_len + 1, bottleneck_bits]

        img_z = mf_output['latent']
        seq_z = mf_output['seq_latent']


        kl_loss = mf_output['kl_loss']
        kl_seq = mf_output['kl_seq_loss']

        # image decoding
        img_decoder_out = self.img_decoder(img_z, trg_char_onehot, seq_z)

        if trg_img is not None:
            img_l1loss = F.l1_loss(img_decoder_out, trg_img)

        ret_dict = {}
        loss_dict = {}

        ret_dict['img'] = {}
        ret_dict['img']['out'] = img_decoder_out
        ret_dict['img']['ref'] = ref_img
        ret_dict['img']['trg'] = trg_img
        
        img_reinput = ret_dict['img']['out'].detach()
        feat_imgre =  self.Feat_Encoder(img_reinput)

        

        if mode in {'train', 'val'}:

            tgt_mask = Variable(subsequent_mask(self.opts.max_seq_len).type_as(ref_pad_mask.data)).unsqueeze(0).expand(
                img_z.size(0), -1, -1, -1).cuda().float()
            command_logits, args_logits, attn = self.transformer_seqdec(x=trg_seq_shifted, memory=latent_feat_seq,
                                                                        trg_char=trg_cls, tgt_mask=tgt_mask)
            

            command_logits_2, args_logits_2 = self.transformer_seqdec.parallel_decoder(command_logits, args_logits,
                                                                                       memory=latent_feat_seq.detach(),
                                                                                       trg_char=trg_cls)
            

            seq_img = self.FTT(feat_imgre,latent_feat_seq.detach())
            command_logits_3, args_logits_3 = self.transformer_seqdec.re_decoder(command_logits_2,args_logits_2, memory=seq_img , trg_char=trg_cls)
            

            total_loss = self.transformer_main.loss(command_logits, args_logits, trg_seq, trg_seqlen, trg_pts_aux)
            total_loss_parallel = self.transformer_main.loss(command_logits_2, args_logits_2, trg_seq, trg_seqlen,
                                                             trg_pts_aux)
            total_loss_re = self.transformer_main.loss(command_logits_3, args_logits_3, trg_seq, trg_seqlen, trg_pts_aux)
            


            vggpt_loss = self.vggptlossfunc(img_decoder_out, trg_img)
            # loss and output
            loss_svg_items = ['total', 'cmd', 'args', 'smt', 'aux']
            # for image
            loss_dict['img'] = {}
            loss_dict['img']['l1'] = img_l1loss
            loss_dict['img']['vggpt'] = vggpt_loss['pt_c_loss']

            # for latent
            loss_dict['kl'] = kl_loss
            loss_dict['kl_seq'] = kl_seq
            loss_dict['nce_img'] = NCELOSS_IMG
            # for svg
            loss_dict['svg'] = {}
            loss_dict['svg_para'] = {}
            loss_dict['svg_re'] = {}
            for item in loss_svg_items:
                ...
                loss_dict['svg_re'][item] = total_loss_re[f'loss_{item}']

            # for img and seq
            loss_dict['imgandseq'] = {}

            loss_dict['imgandseq']['image_feat_loss'] = mf_output['image_feat_loss']
            loss_dict['imgandseq']['seq_feat_loss'] = mf_output['seq_feat_loss']
            loss_dict['imgandseq']['NCEloss'] = mf_output['NCEloss']
            ...
            for t in range(0, trg_len):
                tgt_mask = Variable(subsequent_mask(sampled_svg.size(0)).type_as(ref_seq_cat.data)).unsqueeze(0).expand(
                    sampled_svg.size(1), -1, -1, -1).cuda().float()
                command_logits, args_logits, attn = self.transformer_seqdec(x=sampled_svg, memory=latent_feat_seq,
                                                                            trg_char=trg_cls, tgt_mask=tgt_mask)
            ...
            command_logits_2, args_logits_2 = self.transformer_seqdec.parallel_decoder(cmd_logits=cmd2,
                                                                                       args_logits=arg2,
            ...
            seq_img = self.FTT(feat_imgre,latent_feat_seq.detach())
            #修改了para
            command_logits_3, args_logits_3 = self.transformer_seqdec.re_decoder(cmd_logits=update_command.transpose(0, 1), 
                                                                                    args_logits=update_args.transpose(0, 1), memory=seq_img, trg_char=trg_cls)
            

            prob_comand2 = F.softmax(command_logits_3,-1)
            prob_args2 = F.softmax(args_logits_3,-1)
            re_command2 = torch.argmax(prob_comand2,-1).unsqueeze(-1)
            re_args2 = torch.argmax(prob_args2,-1)
            sampled_svg_re = torch.cat((re_command2, re_args2),-1).transpose(0,1)
            ...
            commands2 = F.one_hot(sampled_svg_re[:, :, :1].long(), 4).squeeze().transpose(0, 1)
            args2 = denumericalize(sampled_svg_re[:, :, 1:]).transpose(0, 1)
    ...
    def fetch_data(self, data, mode):
        ...
        ref_seq = util_funcs.select_seqs(input_sequence, ref_cls, self.opts,
                                         self.opts.dim_seq_short)  # [opts.batch_size, opts.ref_nshot, opts.max_seq_len, opts.dim_seq_nmr]
        ...
        return [ref_img, trg_img], [ref_seq, ref_seq_cat, ref_pad_mask, trg_seq, trg_seq_gt, trg_seq_shifted,
                                    trg_pts_aux], [trg_char_onehot, trg_cls, trg_seqlen]