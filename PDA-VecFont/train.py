...
#from models.mymodel import ModelMain
from models.mymodel3 import ModelMain
...
def train_main_model(opts):
    ...
    # if opts.pretrain_ckpt != None:
    #     path_ckpt = os.path.join('experiments', opts.name_exp, 'checkpoints', opts.pretrain_ckpt)
    #     model_main.load_state_dict(torch.load(path_ckpt)['model'])
    #     print('load ckpt')
    #     print(opts.pretrain_ckpt)
    ...
    parameters_all = [{"params": model_main.img_encoder.parameters()}, {"params": model_main.img_decoder.parameters()},
                        ...
                        {"params": model_main.transformer_seqdec.parameters()},   {"params": model_main.FTT.parameters()},
                        {"params": model_main.Feat_Encoder.parameters()}]
    ...
    for epoch in range(opts.init_epoch, opts.n_epochs):
        for idx, data in enumerate(train_loader):
            ...
            if opts.skip:
                ...
                loss = opts.loss_w_l1 * loss_dict['img']['l1'] + opts.loss_w_pt_c * loss_dict['img']['vggpt'] + opts.kl_beta * loss_dict['kl'] \
                       + loss_dict['svg']['total'] + loss_dict['svg_para']['total'] + loss_dict['imgandseq']['image_feat_loss'] + loss_dict['imgandseq']['seq_feat_loss'] \
                       + opts.kl_beta * loss_dict['kl_seq']  + loss_dict['svg_re']['total']\
                       + loss_dict['imgandseq']['NCEloss'] + loss_dict['nce_img']
            ...
            message = (
                ...
                f"svg_total_loss: {loss_dict['svg_re']['total'].item():.6f}, "
                f"svg_cmd_loss: {opts.loss_w_cmd * loss_dict['svg_re']['cmd'].item():.6f}, "
                f"svg_args_loss: {opts.loss_w_args * loss_dict['svg_re']['args'].item():.6f}, "
                f"svg_smooth_loss: {opts.loss_w_smt * loss_dict['svg_re']['smt'].item():.6f}, "
                f"svg_aux_loss: {opts.loss_w_aux * loss_dict['svg_re']['aux'].item():.6f}, "
                f"image_feat_loss: {loss_dict['imgandseq']['image_feat_loss'] .item():.6f}, "
                f"seq_feat_loss: {loss_dict['imgandseq']['seq_feat_loss'] .item():.6f}, "
                f"NCEloss: {loss_dict['imgandseq']['NCEloss'] .item():.6f}, "
                f"lr: {optimizer.param_groups[0]['lr']:.6f}, "
                f"NCE_img: {loss_dict['nce_img'] .item():.6f}, "
                f"Step: {batches_done}"
            )
            ...
            if opts.freq_val > 0 and batches_done % opts.freq_val == 0:
                ...
                with torch.no_grad():
                    ...
                    if best_l1 > loss_val['img']['l1']:

                        torch.save({'model': model_main.state_dict(), 'opt': optimizer.state_dict(), 'n_epoch': epoch,
                                    'n_iter': batches_done}, f'{dir_ckpt}/best_l1.ckpt')
                        best_l1 = loss_val['img']['l1']
                        print(best_l1)
                        print('save bestl1')
